...
3. **粗粒度多步 rollout**（梯度全程穿过）：
   - 第 k 段：`z = coarse_predictor(z, [â_{kh}..â_{(k+1)h-1}])`
   - 监督：`smooth_l1(z, z_target_{(k+1)h-1})`
4. **细粒度多步 rollout**（per segment, scheduled sampling）：
   - 段起点：k=0 用真实 z_0；k>0 用上一段 coarse 输出（与 planning 一致）
   - 段内逐步：`z = fine_predictor(z, â_t, coarse_cond)`
   - `coarse_cond` 在课程学习下从 EMA target 渐变到 coarse 预测
   - Scheduled sampling：以概率 `1 - curriculum_ratio` 把下一步输入换成 GT
5. **action 正则**：L2 + per-dim variance threshold（防 collapse）
6. **state 正则**（VICReg 风格）：
...
### 5.2 损失
```
L_align    = MSE(action_enc(a_t), â_t)        # 对齐 latent action 空间
L_decode   = MSE(action_dec(â_t), a_t)        # 反向重建
L_cycle    = MSE(action_dec(action_enc(a_t)), a_t)
L_p_enc    = MSE(proprio_dec(z_{0,1,h}), proprio_{0,1,h})       # encoder 输出
L_p_pred   = MSE(proprio_dec(z_{h,1}_pred), proprio_{h,1})      # predictor 输出
L_total    = (L_align + L_decode + 0.5*L_cycle) + L_p_enc + L_p_pred
```

`L_p_pred` 是关键 —— 让 proprio_dec 在 planning 时见到的分布上也准确。
...
### 7.4 为什么需要 multi-step rollout 训练
旧版本只训 z_0 → z_1 一步，但 planning 时 fine 要连续滚 h 步、coarse 要连续滚 K 段，分布外 → compounding error。
新版本训练分布与 planning 一致：fine K*h 步 + coarse K 段都用 chained rollout 监督。

### 7.5 为什么需要 state variance reg
...
```python
{
  ...
  "proprio_dec_state_dict": ...,    # 新增
  "config": vars(args),             # 含 hidden_dim, proprio_hidden_dim
